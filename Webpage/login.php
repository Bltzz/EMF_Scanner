...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        $lietotajvards_err= "Ievadiet lietotājvārdu";
    } else{
        ...
        if($stmt = mysqli_prepare($link, $sql)) { 
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    $lietotajvards_err = "Šāds lietotājs jau ir reģistrēts";
                } else{
                    $lietotajvards = trim($_POST["username"]);
                }
            } else{
                echo "Oops! Neizdevās, reģistrēt lietotāju!";
            }
        }
    ...
    if(empty($lietotajvards_err) && empty($parole_err) && empty($con_parole_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                echo "Oops! Neizdevās, mēģiniet vēlāk!";
            }
        }
         mysqli_stmt_close($stmt);
    }
