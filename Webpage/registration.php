...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        $lietotajvards_err = "Lūdzu, ievadiet lietotājvārdu.";
    } else{
    ...
    if(empty(trim($_POST["password"]))){
        $parole_err = "Lūdzu, ievadiet paroli.";
    } else{
    ...
    if(empty($lietotajvards_err) && empty($parole_err)){
        //dabu baze
        $sql = "SELECT id, username, password FROM lietotaji WHERE username = ?";
                    ...
                    if(mysqli_stmt_fetch($stmt)){
                        if(password_verify($parole, $hashed_password)){
                          ...
                          // Password is correct, so start a new session
                            ...
                            $parole_err = "Nederīga parole.";
                        }
                    }
                } else{
                    // Nav tāds lietotājs
                    $lietotajvards_err = "Izskatas, ka tāds lietotājs nav reģistrēts";
                }
            } else{
                echo "Oops! Neizdevās :( ";
            }
  ...
  <section>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
        <div class="form-group <?php echo (!empty($lietotajvards_err)) ? 'has-error' : ''; ?>">
            ...
            <input type="text" name="username" class="form-control" value="<?php if(isset($_COOKIE["username"])) { echo $_COOKIE["username"]; } ?> <?php echo $lietotajvards; ?>">
            <span class="help-block"><?php echo $lietotajvards_err; ?></span>
        </div>
        <div class="form-group <?php echo (!empty($parole_err)) ? 'has-error' : ''; ?>">
            <label>Password</label>
            <input type="password" name="password" class="form-control"<?php if(isset($_COOKIE["password"])) { echo $_COOKIE["password"]; } ?>>
